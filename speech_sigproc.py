...
class FrontEnd:
    ...
    def frames_to_magspec(self, frames):
        ...
        #mag_frames = ((1.0 / NFFT) * ((mag_frames) ** 2))
    ...
    def magspec_to_fbank(self, magspec):
        ...
        fbank = []
       # print(magspec.shape)
        #print(self.mel_filterbank.shape)
        fbank = np.dot(magspec,self.mel_filterbank.T)
    ...
    def mean_norm_fbank(self, fbank):
        ...
        #print(np.mean(fbank,axis=0))
        #print("hello",fbank.shape)
    ...
    def accumulate_stats(self, fbank):
        self.global_mean += np.sum(fbank,axis=0)
        self.global_var += np.sum(fbank**2,axis=0)
        self.global_frames += fbank.shape[0]
